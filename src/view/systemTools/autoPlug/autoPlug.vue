...
const createPlug = async() => {
  if (!form.plugName || !form.routerGroup) {
    ElMessage.error('分包名称和分包路由组为必填项')
    return
  }
  if (form.hasGlobal) {
  ...
  if (res.code === 0) {
    ElMessageBox('创建成功，分包已自动写入后端plugin目录下，请按照自己的逻辑进行创造')
  }
}
